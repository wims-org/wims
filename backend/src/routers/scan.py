...
async def scan_event(request: Request, body: ScanRequest) -> ScanResponse:
    ...
    await request.app.state.backend_service.append_message_to_all_queues_with_reader(
        ...
        message=SseMessage(data=SseMessage.SseMessageData(reader_id=body.reader_id, rfid=body.tag_id), event=Event.SCAN)
    )
    ...
    if item_raw:
        try:
            ...
            return ScanResponse(
                ...
                item_storage_location=item.container.short_name
                )
